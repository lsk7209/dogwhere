...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info(`Starting simple data collection for region: ${JSON.stringify(region)}`)
    ...
    for (const keyword of searchKeywords) {
      ...
      if (sources.includes('google')) {
        ...
        logger.info(`Collected ${googlePlaces.length} places from Google Places for "${keyword}"`)
      }
      ...
      if (sources.includes('kakao')) {
        ...
        logger.info(`Collected ${kakaoPlaces.length} places from Kakao Map for "${keyword}"`)
      }
    ...
    // 데이터 통합 및 저장
    logger.info(`Total collected places: ${allCollectedPlaces.length}`)
